(function (exports) {
  ...
  function inOwnerDocument(el) {
    ...
    return (
      rootNode instanceof ShadowRoot && el.ownerDocument.contains(rootNode.host)
    );
  }
      ...
      if (target instanceof HTMLElement || target instanceof ShadowRoot) {
        if (
          !document.body.contains(
            target instanceof HTMLElement ? target : target.host
          )
        ) {
  ...
  const characterDataSetData$1 = getDescriptor$2(
    characterDataProto$1,
    "data"
  ).set;
  ...
  class VSimpleNode {
    ...
    moveBeforeVNode(other, afterNode) {
      nodeInsertBefore$2.call(
        this.parentEl,
        this.el,
        other ? other.el : afterNode
      );
    }
    beforeRemove() {}
    remove() {
  ...
  const NO_CALLBACK = () => {
    throw new Error(
      "Called NO_CALLBACK. Owl is broken, please report this to the maintainers."
    );
  };
  ...
  function onWillUnmount(fn) {
    const node = getCurrent();
    const decorate = node.app.dev ? wrapError : (fn) => fn;
    node.willUnmount.unshift(
      decorate(fn.bind(node.component), "onWillUnmount")
    );
  }
  ...
  function prepareList(collection) {
    ...
    if (Array.isArray(collection)) {
      ...
      throw new OwlError(
        `Invalid loop expression: "${collection}" is not iterable`
      );
    }
  ...
  function validateProps(name, props, comp) {
    const ComponentClass =
      typeof name !== "string" ? name : comp.constructor.components[name];
    if (!ComponentClass) {
  ...
  class TemplateSet {
    constructor(config = {}) {
      ...
      if (config.templates) {
        if (
          config.templates instanceof Document ||
    ...
    getTemplate(name) {
      ...
      if (!(name in this.templates)) {
        const rawTemplate =
          ((_a = this.getRawTemplate) === null || _a === void 0
            ? void 0
            : _a.call(this, name)) || this.rawTemplates[name];
    ...
    callTemplate(owner, subTemplate, ctx, parent, key) {
      ...
      return toggler(
        subTemplate,
        template.call(owner, ctx, parent, key + subTemplate)
      );
    }
  }
  // -----------------------------------------------------------------------------
  ...
  class CodeGenerator {
    ...
    insertBlock(expression, block, ctx) {
      ...
      if (block.parentVar) {
        ...
        this.addLine(
          `${block.parentVar}[${ctx.index}] = withKey(${blockExpr}, ${key});`
        );
        return;
      }
      if (ctx.tKeyExpr) {
    ...
    compileTDomNode(ast, ctx) {
      ...
      if (ast.ref) {
        ...
        let name = `\`${ast.ref}\``;
        if (isDynamic) {
          name = replaceDynamicParts(ast.ref, (expr) =>
      ...
      if (isNewBlock) {
        ...
        if (block.children.length && block.hasDynamicChildren) {
          ...
          this.addLine(
            `let ${block.children.map((c) => c.varName).join(", ")};`,
            codeIdx
          );
        }
      }
    ...
    compileTEsc(ast, ctx) {
      ...
      if (ast.expr === "0") {
        ...
        if (ast.defaultValue) {
          ...
          expr = `withDefault(${expr}, ${toStringExpression(
            ast.defaultValue
          )})`;
        }
      }
    ...
    compileTForeach(ast, ctx) {
      ...
      if (!ast.hasNoLast) {
        this.addLine(
          `ctx[\`${ast.elem}_last\`] = ${loopVar} === ${keys}.length - 1;`
        );
      }
      if (!ast.hasNoIndex) {
      ...
      if (this.dev) {
        ...
        this.addLine(
          `keys${block.id}.add(String(key${this.target.loopLevel}));`
        );
      }
      let id;
      if (ast.memo) {
  ...
  function parseDOMNode(node, ctx) {
    ...
    let ns =
      !ctx.nameSpace && ROOT_SVG_TAGS.has(tagName)
    ...
    if (node.tagName !== "t") {
      ...
      if (ast && ast.type === 11 /* TComponent */) {
        return {
          ...ast,
          slots: {
            default: { content: tcall, scope: null, on: null, attrs: null },
          },
        };
      }
    }
    ...
    for (const d of ["t-esc", "t-out"]) {
      ...
      for (const el of elements) {
        if (el.childNodes.length) {
          throw new OwlError(
            `Cannot have ${d} on a component that already has content`
          );
        }
